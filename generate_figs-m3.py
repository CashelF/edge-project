...
def fetch_file(hw_type, device_number, filename, local_path):
    ...
    # build correct *remote* path with forward‑slashes
    # and include the device_logs subfolder on the Pi
    remote_file = posixpath.join(remote_path, filename)
    print(f"Fetching remote file: {remote_file} from {host}")
    ...
    try:
        ...
        print(f"[!] Failed to fetch {remote_file!r}: {e}")
        return -1
    finally:
        scp.close()
...
def plot_acc_loss(experiments, runs, exp_labels, title, time=True, m3=False):
    # Accuracy
...
parser.add_argument('--m3', action='store_true', help='')
...
if __name__ == '__main__':
        ...
        for device, dev_idx in zip([0, 1], device_numbers):
            for rs in range(1, r+1):
                ...
                if result == -1:
                    ...
                    exit(-1)
    if args.accuracy:
        plot_acc_loss(exps, runs, exp_labels, args.plot_title, args.time, args.m3)
    ...
    for exp, myrun in zip(exps, runs):
        ...
        if args.m3:
            for r in range(1, myrun + 1):
                avg_powers, avg_energies, avg_data_communicated = plot_power_energy_communication(exp=exp, run=r, power=args.power, energy=args.energy, communication=args.communication)
                print(f"\nExperiment {exp} Run {r}:")
